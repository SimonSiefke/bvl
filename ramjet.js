;(function(global, factory) {
  ...
  if (typeof CSS2Properties !== 'undefined') {
    ...
    styleKeys = Object.keys(document.createElement('div').style).filter(
      function(k) {
      ...
      if (destinationIsFixed) {
        ...
        if (
          ...
          clone.style.top =
            top -
            parseInt(style.marginTop, 10) -
            (offsetParentBcr.top - parseInt(offsetParentStyle.marginTop, 10)) +
            'px'
          clone.style.left =
            left -
            parseInt(style.marginLeft, 10) -
            (offsetParentBcr.left -
              parseInt(offsetParentStyle.marginLeft, 10)) +
            'px'
        }
      }
  ...
  function transformers_TimerTransformer___classCallCheck(
    instance,
    Constructor
  ) {
  ...
  var transformers_TimerTransformer__TimerTransformer = function TimerTransformer(
    from,
    to,
    options
  ) {
    transformers_TimerTransformer___classCallCheck(
      this,
      transformers_TimerTransformer__TimerTransformer
    )
    ...
    function tick() {
      ...
      // border radius
      var fromBorderRadius = utils_getBorderRadius(
        from.borderRadius,
        to.borderRadius,
        dsxf,
        dsyf,
      ...
      var toBorderRadius = utils_getBorderRadius(
        to.borderRadius,
        from.borderRadius,
        dsxt,
        dsyt,
      ...
      var fromTransform =
        utils_getTransform(from.isSvg, cx, cy, dx, dy, dsxf, dsyf, t, t_scale) +
        ' ' +
        from.transform
      var toTransform =
        utils_getTransform(
  ...
  function transformers_KeyframeTransformer___classCallCheck(
    instance,
    Constructor
  ) {
    if (!(instance instanceof Constructor)) {
  ...
  var transformers_KeyframeTransformer__KeyframeTransformer = function KeyframeTransformer(
    from,
    to,
    options
  ) {
    transformers_KeyframeTransformer___classCallCheck(
      this,
      transformers_KeyframeTransformer__KeyframeTransformer
    )
  ...
  function getKeyframes(from, to, options) {
    ...
    function addKeyframes(pc, t, t_scale) {
      ...
      var fromTransform =
        utils_getTransform(false, cx, cy, dx, dy, dsxf, dsyf, t, t_scale) +
        ' ' +
        from.transform
      var toTransform =
        utils_getTransform(
  ...
  var ramjet = {
    ...
    show: function() {
      for (
        var _len2 = arguments.length, nodes = Array(_len2), _key2 = 0;
        _key2 < _len2;
        _key2++
      ) {
